...
{
    public class CreateCoursePrerequisiteCommandHandler : IRequestHandler<CreateCoursePrerequisiteCommand, CourseDto>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public CreateCoursePrerequisiteCommandHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<CourseDto> Handle(CreateCoursePrerequisiteCommand request, CancellationToken cancellationToken)
        {
            ...
            if (exists)
                throw new ValidationException(new List<string> {
                    $"Prerequisite relationship already exists between '{course.Code}' and '{prerequisite.Code}'."
                });
            ...
            if (hasCircular)
                throw new ValidationException(new List<string> {
                    $"Circular dependency detected: '{prerequisite.Code}' depends on '{course.Code}'."
                });

            // ─── 6. Create Prerequisite Mapping ─────────────────────────────
            ...
            // ─── 7. Return updated course with prerequisites ─────────────────
