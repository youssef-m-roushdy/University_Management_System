...
namespace University_Management_System.Application.Handlers.Instructors
{
    public class CreateInstructorCommandHandler : IRequestHandler<CreateInstructorCommand, InstructorDto>
    {
        ...
        public CreateInstructorCommandHandler(IUnitOfWork unitOfWork, IUserService userService, IMapper mapper)
        {
        ...
        public async Task<InstructorDto> Handle(CreateInstructorCommand request, CancellationToken cancellationToken)
        {
            ...
            // ─── 2. Create User with "Instructor" role ──────────────────────
            var user = await _userService.CreateUserAsync(request.Dto, "Instructor");
