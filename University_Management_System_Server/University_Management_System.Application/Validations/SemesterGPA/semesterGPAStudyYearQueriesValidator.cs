...
{
    public class semesterGPAStudyYearQueriesValidator : AbstractValidator<semesterGPAStudyYearQueries>
    {
        public semesterGPAStudyYearQueriesValidator()
        {
            ...
            // Semester Title
            RuleFor(x => x.SemesterTitle)
                .IsInEnum().WithMessage("Invalid semester title value")
                .When(x => x.SemesterTitle.HasValue);

            // Sort By Allowed Values
            RuleFor(x => x.SortBy)
                .Must(BeAllowedSortField)
                .WithMessage("Invalid sort field. Allowed fields: student, academiccode, gpa, credithours, semester, calculatedat")
